...
class TEST_Sensitivity(unittest.TestCase):
    ...
    #-------------------------------------------------#
    def test_calc_Sensitivity_Sets(self):
        # Check binary score
        score_bi = calc_Sensitivity_Sets(self.gt_bi, self.pd_bi, c=1)
        ...
        self.assertTrue(callable(metric_dict["TPR"]))
        self.assertTrue("TruePositiveRate" in metric_dict)
        self.assertTrue(callable(metric_dict["TruePositiveRate"]))
        self.assertTrue("Recall" in metric_dict)
        self.assertTrue(callable(metric_dict["Recall"]))

    #-------------------------------------------------#
    #          Calculate : Sensitivity via CM         #
    #-------------------------------------------------#
    def test_calc_Sensitivity_CM(self):
        # Check binary score
        score_bi = calc_Sensitivity_CM(self.gt_bi, self.pd_bi, c=1)
        self.assertTrue(isinstance(score_bi, np.float64))
        self.assertTrue(score_bi == calc_Sensitivity(self.gt_bi, self.pd_bi, c=1))
        # Check multi-class score
        for i in range(5):
            score_mc = calc_Sensitivity_CM(self.gt_mc, self.pd_mc, c=i)
            self.assertTrue(isinstance(score_mc, np.float64))
            self.assertTrue(score_mc == calc_Sensitivity(self.gt_mc, self.pd_mc, c=i))
